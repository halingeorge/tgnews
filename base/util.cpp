...
std::vector<Document> MakeDocumentsFromDir(const std::string& dir, int nDocs) {
  ...
  for (auto it = start; it != end; it++) {
    ...
    if (path.substr(path.length() - 5) == ".html") {
      ...
      documents.emplace_back(tgnews::Document(it->path().filename().string(), std::move(content), 0, tgnews::Document::State::Added));
    }
